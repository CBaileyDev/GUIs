...
class MenuApp(ctk.CTk):
    def __init__(self) -> None:
        ...
        ctk.CTkLabel(card, text="CustomTkinter Menu",
                     font=ctk.CTkFont(size=22, weight="bold"), text_color="#ffffff").pack(anchor="w", padx=22, pady=(20, 0))
        ctk.CTkLabel(card, text="Rounded widgets on a dark card",
                     font=ctk.CTkFont(size=12), text_color="#8a93b3").pack(anchor="w", padx=22, pady=(0, 14))
        ...
        ctk.CTkButton(btn_row, text="Apply", fg_color="#5b6cff", hover_color="#6d7dff",
                      corner_radius=8, height=38, command=lambda: self.log("Apply clicked")
                      ).pack(side="left", expand=True, fill="x", padx=(0, 6))
        ctk.CTkButton(btn_row, text="Reset", fg_color="#252b45", hover_color="#2d3352",
        ...
        ctk.CTkLabel(slider_head, text="Opacity", font=ctk.CTkFont(size=13), text_color="#c3c9db").pack(side="left")
        self.slider_value = ctk.CTkLabel(slider_head, text="72%",
                                         font=ctk.CTkFont(size=13, weight="bold"), text_color="#5b6cff")
        self.slider_value.pack(side="right")

        self.slider = ctk.CTkSlider(card, from_=0, to=100, number_of_steps=100,
        ...
        ctk.CTkCheckBox(card, text="High DPI scaling", variable=self.check_var,
                        fg_color="#5b6cff", hover_color="#6d7dff", corner_radius=4,
                        font=ctk.CTkFont(size=13),
                        command=lambda: self.log(f"DPI scaling: {bool(self.check_var.get())}")
                        ).pack(anchor="w", padx=22, pady=(0, 14))

        self._section_label(card, "Appearance")
        self.combo = ctk.CTkComboBox(card, values=["Dark", "Light", "System"],
                                     fg_color="#0f1324", border_color="#383f5c",
                                     button_color="#5b6cff", button_hover_color="#6d7dff",
                                     dropdown_fg_color="#161b2e",
                                     command=lambda v: self.log(f"Appearance: {v}"))
        self.combo.set("Dark")
        self.combo.pack(fill="x", padx=22, pady=(0, 14))

        self.entry = ctk.CTkEntry(card, placeholder_text="Profile name…",
                                  fg_color="#0f1324", border_color="#383f5c", corner_radius=8, height=36)
        self.entry.pack(fill="x", padx=22, pady=(0, 14))
        self.entry.bind("<KeyRelease>", lambda _e: self.log(f"Profile: {self.entry.get()}"))

        self.status = ctk.CTkLabel(card, text="Ready",
                                   font=ctk.CTkFont(size=12, family="Menlo"),
                                   fg_color="#0b0f1e", text_color="#8a93b3",
                                   corner_radius=8, anchor="w", padx=12, pady=10)
    ...
    def _section_label(self, parent, text: str) -> None:
        ctk.CTkLabel(parent, text=text, font=ctk.CTkFont(size=11, weight="bold"),
                     text_color="#8a93b3").pack(anchor="w", padx=22, pady=(10, 6))
