...
QLabel#status {
    background: #15161d; border: 1px solid #2a2c3a; border-radius: 6px;
    padding: 10px 12px; color: #9098b0;
    font-family: "Menlo", "Consolas", monospace; font-size: 12px;
}
QFrame#divider { background: #2a2c3a; max-height: 1px; min-height: 1px; }
QPushButton { padding: 10px 14px; border-radius: 7px; font-weight: 600; font-size: 13px; border: none; }
QPushButton#primary { background: #6c63ff; color: #ffffff; }
QPushButton#primary:hover { background: #7d75ff; }
QPushButton#secondary { background: #2a2c3a; color: #e8eaf2; border: 1px solid #383b4c; }
QPushButton#secondary:hover { background: #33364a; }
QSlider::groove:horizontal { border: none; height: 4px; background: #2a2c3a; border-radius: 2px; }
...
QCheckBox { color: #e8eaf2; font-size: 14px; spacing: 10px; }
QCheckBox::indicator {
...
class MenuWindow(QWidget):
    def __init__(self) -> None:
        ...
        actions.setProperty("class", "sectionTitle")
        ...
        sl_lbl.setProperty("class", "formLabel")
        ...
        self.check = QCheckBox("Show toolbar")
        self.check.stateChanged.connect(lambda s: self.log(f"Toolbar: {bool(s)}"))
        root.addWidget(self.check)

        combo_lbl = QLabel("Style")
        combo_lbl.setProperty("class", "formLabel")
        root.addWidget(combo_lbl)
        combo = QComboBox()
        combo.addItems(["Fusion", "Windows", "macOS"])
        combo.currentTextChanged.connect(lambda t: self.log(f"Style: {t}"))
        root.addWidget(combo)

        entry = QLineEdit()
        entry.setPlaceholderText("Window title…")
